import { Bed, Bath, Heart, Check, X, Sparkles, Shield, PawPrint, DollarSign, MapPin, Users, Shirt, Star, Home, Wand2 } from "lucide-react";
...
export function ListingCard({ 
  ...
  const getSmartChips = (): SmartChip[] => {
    ...
    // Pet policy chip
    if (id === 2 || id === 4) { // Mock: some listings don't allow pets
      chips.push({
        type: 'warning',
        icon: X,
            ...
            <div className="flex flex-wrap gap-1.5">
              {smartChips.map((chip, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`px-2.5 py-1 rounded-full text-xs font-medium flex items-center gap-1 transition-all duration-200 hover:scale-105 ${getChipStyles(chip.type)}`}
                  >
